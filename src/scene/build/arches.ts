...
export function buildMainArches(scene: LedScene, data: MainArches): void {
  for (const arch of data.arches) {
    ...
    arch.pixel_polygons.forEach((poly, i) => {
      ...
      scene.mesh('mainArches', outline, [0, 0, arch.x_position_m], led);
    });
  }
}
...
export function buildMiniArches(scene: LedScene, left: MiniArches, right: MiniArches): void {
  for (const arch of [...left.arches, ...right.arches]) {
    ...
    scene.points('miniArches', rollLeds(worlds, arch.universe, 1), { circular: false, size: 0.25 });
  }
}
...
export function buildQuadArches(scene: LedScene, quads: QuadArches[]): void {
  for (const quad of quads) {
    for (const face of quad.faces) {
      ...
      scene.points('quadArches', rollLeds(worlds, face.universe, face.universe_start_channel), {
        circular: false,
        size: 0.25,
      });
    }
